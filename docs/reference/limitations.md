...
- `ip_allowlist.trust_proxy_headers` trusts the first client address in `Forwarded` or `X-Forwarded-For`; enable it only when the control API is reachable exclusively through a trusted proxy.
