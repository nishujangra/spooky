...
mod tests {
    ...
    async fn test_channel_body_multiple_chunks() {
        ...
        // Frame::data() convierte Bytes en Frame
    ...
    async fn test_channel_body_empty() {
        ...
        // Si no hay datos y el sender está vivo, debe devolver Pending
        match body.as_mut().poll_frame(&mut cx) {
            Poll::Pending => {}
    ...
    #[tokio::test]
    async fn test_channel_body_check_error_type() {
        let (_tx, body) = ChannelBody::channel(16);
        let body = Pin::new(body);

        // Verificar que el tipo de error es Infallible
        let _: hyper::body::BodyError = body;
        // Si compila, el tipo de error es correcto
    }
}
